  ...
  def scrap_course_items(self, page_content: str) -> [dict]:
    ...
    for row in courses.findAll('div', attrs = {'class':'courseblock'}):
      ...
      if unitText.find('Workload Units') != -1:
        unitText = splitText[2].replace("Workload Units", '').replace(" ", "")
      ...
      for nextSibling in nextSiblings:
        if re.findall('\n', nextSibling.text) and 'Corequisite' in nextSibling.text:
          ...
          course_details['same_as'] = nextSibling.text.partition('as')[2].replace('\xa0','').replace('\n','')
        elif nextSibling.text.startswith('Restriction'):
          course_details['restriction'] = nextSibling.text.partition(':')[2].replace('\xa0','').replace('\n','')
        elif nextSibling.text.startswith('Concurrent'):
          course_details['concurrent'] = nextSibling.text.partition('with')[2].replace('\xa0','').replace('\n','')
        elif nextSibling.text.startswith('Repeatability'):
          course_details['repeatability'] = nextSibling.text.partition(':')[2].replace('\xa0','').replace('\n','')
        elif nextSibling.text.startswith('Grading'):
          course_details['gr_option'] = nextSibling.text.partition(':')[2].replace('\xa0','').replace('\n','')
        elif nextSibling.text.startswith('Overlaps'):
          course_details['overlaps_with'] = nextSibling.text.partition('with')[2].replace('\xa0','').replace('\n','')
