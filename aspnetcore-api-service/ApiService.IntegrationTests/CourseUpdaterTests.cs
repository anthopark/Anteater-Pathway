...
using ApiService.BackgroundTasks.CourseDataUpdate;
...
namespace ApiService.IntegrationTests
{
    ...
    public class CourseUpdaterTests : IntegrationTestsBase
    {
        ...
        // private const string _catalogueDataDirectoryPath = "../../../../AnteaterPathwayAPI/CourseDataScripts/course-catalogue-data/";
        ...
        [Fact(DisplayName =
            "CatalogeDataUpdater.Update() Inserts all new course items from csv files when DB is empty")]
        public async Task InsertsAllNewCourse_WhenDbIsEmpty()
        {
            await _catalogueDataUpdater.Update();

            // get around the inconsistent bug from Mongo2go
            Thread.Sleep(500);
            ...
            var expectedFileCount = Directory.GetFiles(_catalogueDataDirectoryPath).Length;

            Assert.Equal(expectedFileCount, _catalogueDataUpdater.CsvFileCount);
