...
def extract_ge_category(ge_info: ['strong tag']) -> str:
    if len(ge_info) == 0:
        return ''
    result = unidecode(ge_info[0].text.strip('(). '))
    if '(' in result:
        result = result + ')'
    elif ')' in result:
        result = '(' + result
...
def handle_not_separated_by_p_tag_issue(descriptions: ['p tag']) -> [str]:
    ...
    for desc in descriptions:
        if desc.find('br'):
            corrected = [fix.strip() for fix in unidecode(
                desc.text).split('\n') if fix != '']
...
if __name__ == "__main__":
    ...
    for dept, url in dept_url_dict.items():
        ...
        with open(DATA_SAVE_DIR / f'{dept}.csv', 'w') as write_file:
            ...
            for course_div in course_divs:
                ...
                descriptions = handle_not_separated_by_p_tag_issue(
                    descriptions[1:])

                for extra_desc in EXTRA_DESCRIPTIONS:
                    course_data.append(extract_extra_desc(
                        descriptions, extra_desc))
